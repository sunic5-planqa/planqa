...
짧게 안내하고, "그럼 1번부터 같이 시작해볼게요"처럼 바로 다음 행동(보통 clone/설치)으로
이어가라. 설치/실행 방법과 브랜치·커밋·PR 흐름을 한 번에 전부 나열하지 않는다 —
`README.md`(설치/실행)와 `CONTRIBUTING.md`(브랜치·PR 흐름)가 원본이니 `docs/onboarding/`에
미리 베껴두지 않고, 실제 진행되는 단계에 맞춰 그때그때 필요한 만큼만 읽어서 안내해라.
"막히면 물어보세요" 같은 일반적인 문구로 끝내지 말고, PR을 열 단계가 되면 "직접 push가
안 되니 PR을 열고 혜서/가영님께 merge 요청을 드리세요"라고 안내해라.

이 사람이 "진행해줘"처럼 실제 코드 작업을 시켜도, 바로 구현부터 들어가지 마라. 먼저
계획을 세워서 본인에게 보여주고 승인을 받은 뒤에만 구현을 시작해라 — Claude Code
세션이면 `EnterPlanMode`로 실제 plan mode에 들어가고, 승인 전에는 파일을 고치지 않는다.
plan mode가 없는 도구(Codex 등)에서는 똑같은 목적으로, 계획을 텍스트로 먼저 제시하고
명시적인 승인(진행해도 된다는 답)을 받은 뒤에만 진행해라.
