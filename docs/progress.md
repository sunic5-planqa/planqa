...
넘버링(번호 누락/중복/순서 오류, 애매한 계층 구조)을 규칙 기반으로 검사하는 단계를 조건부로
끼워넣었다 — 오류가 없으면 지금처럼 추가 화면 없이 바로 넘어가고, 있을 때만 확인 화면이 뜬다.
`EnterPlanMode`로 계획을 세우고(Explore 에이전트 3개 병렬 조사 + Plan 에이전트 1개로 설계 검증),
승인 과정에서 사용자가 판정 기준 명확화/번호만 치환 보장/적용 후 재검증 3가지를 요구해 계획을
보완한 뒤 구현. 계획: `/Users/song-eunseong/.claude/plans/wondrous-orbiting-lecun.md`.

- **핵심 설계 결정**: 원문 헤딩의 번호는 기존 시스템(`_build_heading_numbers`)이 이미 "작성자마다
  제각각이라 신뢰 불가"라며 무시하고 등장 순서로 재계산해 쓰고 있었다 — 이건 이슈 위치 표시용이지
  번호 자체를 검증하는 기능이 아니었다. 이번 기능은 정반대로 원문에 실제로 적힌 번호를 검사
  대상으로 삼아야 해서, `parse_document`/리뷰 파이프라인을 전혀 쓰지 않는 완전히 독립된 신규
  모듈(`qa_engine/numbering_validation.py`)로 구현 — AI QA 판단 영역과 안 섞였다.
- **판정 알고리즘**(정규식+스택 기반, LLM/AI 호출 없음, 결정적): 헤딩 레벨(`#`개수)을 위계의
  진실로 신뢰해 구조 트리를 만들고, 각 헤딩에서 작성자가 적은 번호를 추출한다. 번호의 dot-prefix가
  구조적 부모의 실제 번호와 다르면 🟡 확인 필요(계층 모호)로 분류하고 그룹핑에서 제외. 나머지는
  같은 부모를 가진 형제끼리 묶어 기대 순번(1,2,3...)과 비교하고, 그 그룹 전체 집합을 기준으로
  값이 2번 이상 나오면 번호 중복, 기대값이 전혀 없으면 번호 누락, 그 외엔 번호 순서 오류로
  라벨링(🟢 자동 수정 가능) — 이 "전체 집합 기준" 규칙 덕분에 번호가 밀리며 연쇄적으로 어긋나는
  뒤쪽 항목들도 각자 독립적으로, 위치와 무관하게 결정적으로 판정된다. `before_text`/`after_text`는
  헤딩 라인 전체 텍스트에서 번호 캡처 그룹만 `re.sub`로 치환해 만들어 제목/구분자/공백을 코드
  레벨에서 보존한다.
- **재검증 흐름(사용자 피드백 3번 반영)**: 자동 수정은 기존 `APPLY_ISSUE_EDIT` 메시지(content
  script의 storage-HTML 치환 → Confluence PUT)를 그대로 재사용 — 새 저장 경로를 안 만들었다.
  단, 백엔드가 보관 중인 `Document.raw_text`는 확장이 직접 고치는 실제 문서와 별개라, 적용 후
  그대로 재검증하면 여전히 옛 상태를 본다. 그래서 `POST /qa-jobs/{job_id}/numbering-issues/apply`
  를 신설해 실제 반영된 `{before_text, after_text}` 쌍으로 백엔드의 `raw_text`도 동기화하고, 그
  자리에서 다시 `validate_numbering()`을 돌려 남은 오류를 돌려준다. 프론트는 남은 오류가 있으면
  검토 종료로 넘어가지 않고 같은 화면에 갱신된 목록을 다시 보여준다(무조건 나갈 수 있는 건
  "수정하지 않고 검토 종료하기" 버튼뿐).
- 신규: `models/numbering_issue.py`, `qa_engine/numbering_validation.py`,
  `components/screens/NumberingCheckScreen.tsx`, `state/numberingChecklist.ts`(기본 체크 상태
  파생 순수 함수, 컴포넌트 테스트 인프라가 없어 이쪽만 단위테스트). 수정: `api/qa_jobs.py`(라우트
  2개만 얇게 추가, 기존 QA 파이프라인/`_build_heading_numbers` 무변경), `IssueListScreen.tsx`(QA
  완료 버튼 onClick만 교체), `App.tsx`/`appReducer.ts`/`types.ts`/`api/client.ts`/`api/types.ts`
  (새 화면·액션·타입 배선), `global.css`(신규 화면 스타일만 추가). `HistoryExportScreen.tsx`와
  QA 리뷰 파이프라인은 전혀 안 건드림.
- 검증: 백엔드 `uv run pytest` **169개** 전부 통과(신규 11개 — 누락/중복/순서오류/계층모호/
  구두점보존/미번호헤딩 트레이스 그대로 assert), `ruff check` 클린. 프론트
  `typecheck`/`lint`/`test`(vitest **115개**, 신규 4개)/`prettier` 전부 통과.

### Next

- **Claude가 검증 불가능한 것**: 실제 Confluence 페이지에서 QA 완료 → 넘버링 오류 화면 →
  선택 항목 적용 → 복제 페이지에 번호만 바뀌는지, 재검증 후 남은 오류가 화면에 다시 뜨는지 —
  은성님 직접 확인 필요(백엔드가 없어 로컬 서버도 아직 못 띄워봄 — `uv`를 이번 세션에서 새로
...
git commit 없이 워킹 디렉토리에 uncommitted 상태로만 남아있었다(pull/checkout과 무관하게 로컬
디스크에는 그대로 있었음). "다시 넣어달라"는 요청을 받고 처음엔 새로 구현하려 했으나, `EnterPlanMode`
탐색 단계에서 파일이 이미 존재한다는 걸 확인 — 사용자에게 확인받은 뒤 새로 짜지 않고 기존 구현을
스펙 21개 항목과 하나하나 대조 검증하는 쪽으로 방향을 바꿨다.

- 대조 결과 스펙과 불일치하는 부분 없음(판정 케이스 A/B/C, 계층 모호 처리, before/after 번호만
  치환, 복제본 페이지 최신 본문 추출 순서, 재검증 실패 시 화면 유지 등 전부 기존 코드가 이미 충족).
  코드 변경 없이 커밋만 진행.
